...
class CouplingTransform(Transform):
    ...
    def forward(self, inputs, context=None):
        #print("inputs, context in coupling.py: ", inputs.shape, context.shape)
    ...
    def inverse(self, inputs, context=None):
        ...
        identity_split = inputs[:, self.identity_features, ...]
        transform_split = inputs[:, self.transform_features, ...]
        #print("transform_split : ", transform_split.shape, torch.min(transform_split).item(), torch.max(transform_split).item())
        logabsdet = 0.0
        if self.unconditional_transform is not None:
        ...
        #print("output : ", transform_split.shape, torch.min(transform_split).item(), torch.max(transform_split).item())
